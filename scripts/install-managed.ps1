...
Mary — managed (administrator) deployment · Windows
  ...
  Run from an ELEVATED PowerShell, inside the repo:
    powershell -ExecutionPolicy Bypass -File scripts\install-managed.ps1 [-AllowManagedHooksOnly] [-Force]
...
$maryHooks = @{
  ...
  PostToolUseFailure = @(@{ matcher = $WriteTools; hooks = @($recorder) })
  PermissionDenied   = @(@{ matcher = $WriteTools; hooks = @($recorder) })
  Notification       = @(@{ matcher = 'permission_prompt'; hooks = @($notifier) })
  SessionStart       = @(@{ matcher = '*'; hooks = @($report) })
}
