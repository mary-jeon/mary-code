...
저장되는 설명·요청 사본은 **시크릿 마스킹**을 거칩니다(`Authorization:` 헤더, `key=value`
할당, AWS·GitHub·Slack·JWT 토큰 형태를 기록 전에 치환하고, 마스킹된 항목에는
`redacted: true`가 붙습니다). 원장은 영구 평문이라, 게이트에 걸린 `curl -H "Authorization:
Bearer …"`가 아니면 명령이 끝난 뒤에도 토큰을 품고 남기 때문입니다. 해시는 원본 입력으로
계산되므로 마스킹이 승인→결과 대조를 깨지 않고, 사람이 보는 대화상자는 마스킹 전 원문입니다.
마스킹은 패턴 기반 최선-노력이며, 인식 못 하는 시크릿 형태는 평문으로 남습니다(위협 모델 참조).
